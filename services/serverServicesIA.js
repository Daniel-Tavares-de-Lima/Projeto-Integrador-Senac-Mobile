...
// // Chave da API Gemini (armazenar em .env na produção)
// const GEMINI_API_KEY = 'AIzaSyAvSGkHxRKFNkhHGyJisrQ8cs7U-sbDKR0';
// const GEMINI_API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-pro:generateContent?key=${GEMINI_API_KEY}`;

// export const generateReport = async (caseData, victims, peritos) => {
//   try {
//     // Montar prompt
//     let promptText = `Gere um laudo pericial preliminar conciso e bem estruturado para o seguinte caso:\n\n`;
//     promptText += `**ID do Caso**: ${caseData.id}\n`;
//     promptText += `**Tipo de Caso**: ${caseData.classification || 'Não especificado'}\n`;
//     promptText += `**Data do Fato**: ${caseData.dateFact || 'Não informada'}\n`;
//     promptText += `**Descrição**: ${caseData.description || 'Sem descrição'}\n`;
//     promptText += `**Responsável**: ${
//       peritos.find(p => p.id === caseData.managerId)?.name || 'Não informado'
//     }\n`;
//     promptText += `**Solicitante**: ${caseData.solicitante || 'Não informado'}\n`;
//     promptText += `**Status**: ${caseData.statusCase || 'Não informado'}\n\n`;

//     if (victims.length > 0) {
//       promptText += `**Vítimas**:\n`;
//       victims.forEach(v => {
//         promptText += `- Nome: ${v.name || 'Desconhecido'}, ID: ${v.id}, Gênero: ${v.gender || 'Não informado'}\n`;
//       });
//       promptText += `\n`;
//     }

//     const content = JSON.parse(caseData.content || '{}');
//     if (content.requestedExams?.length > 0) {
//       promptText += `**Exames Solicitados**:\n`;
//       content.requestedExams.forEach(exam => {
//         promptText += `- ${exam.exam} (Solicitado em: ${new Date(exam.date).toLocaleDateString('pt-BR')})\n`;
//       });
//       promptText += `\n`;
//     }

//     promptText += `Com base nessas informações, gere um laudo técnico em português com:\n`;
//     promptText += `- Título: Um título claro e objetivo (ex.: "Laudo Pericial - Caso [ID]").\n`;
//     promptText += `- Conteúdo: Estruturado com seções (Introdução, Análise, Conclusão), analisando os dados e sugerindo linhas de investigação.\n`;
//     promptText += `Retorne o laudo em formato JSON com os campos "title" e "content".`;
...
//       } catch (e) {
//         throw new Error('Resposta da Gemini não está em formato JSON válido.');
//       }
//     } else {
...
//     let promptText = `Gere um laudo pericial preliminar conciso e bem estruturado para o seguinte caso:\n\n`;
//     promptText += `**ID do Caso**: ${caseData.id}\n`;
//     promptText += `**Tipo de Caso**: ${
//       caseData.classification || "Não especificado"
//     }\n`;
//     promptText += `**Data do Fato**: ${
//       caseData.date_fact || caseData.dateFact || "Não informada"
//     }\n`;
//     promptText += `**Descrição**: ${caseData.description || "Sem descrição"}\n`;
//     promptText += `**Responsável**: ${
//       peritos.find((p) => p.id === caseData.managerId)?.name || "Não informado"
//     }\n`;
//     promptText += `**Solicitante**: ${
//       caseData.solicitant || caseData.solicitante || "Não informado"
//     }\n`;
//     promptText += `**Status**: ${caseData.statusCase || "Não informado"}\n\n`;
...
//           } catch (e) {
//             throw new Error(
//               "Resposta da Gemini não está em formato JSON válido."
//             );
//           }
//         } else {
//           throw new Error("Resposta da Gemini não contém o conteúdo esperado.");
...
//         ) {
//           console.warn(
//             `Erro 429 na tentativa ${attempt}. Aguardando ${delay}ms antes de tentar novamente.`
//           );
//           await sleep(delay);
...
**Data do Fato**: ${caseData.date_fact || caseData.dateFact || "Não informada"}
...
Retorne apenas o texto em Markdown, sem delimitadores de código (como \`\`\`) ou qualquer outro formato.`;
